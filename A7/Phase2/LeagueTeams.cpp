...
void LeagueTeams::adding_this_week_info(const vector<shared_ptr<InfoOfPlayer>> &team_one, string week_num)
{
    for (int i = 0; i < team_one.size(); i++)
    {
        for (int j = 0; j < players.size(); j++)
        {
            if (players[j]->has_the_same_name(team_one[i]->get_name()))
            {
               players[j]->add_the_score_of_this_player(week_num,team_one[i]->get_score(),
               team_one[i]->get_reaching_goals(),team_one[i]->get_assisting_goals(),team_one[i]->get_clean_sheet());
            }
        }
    }
}
void LeagueTeams::the_info_right_now(string week_num)
{
    for (int i = 0; i < players.size(); i++)
    {
        players[i]->the_info_right_now(week_num);
    }
